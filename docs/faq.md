...
1. Run `subrosa embed` once. The first run downloads the model it uses ([`BAAI/bge-small-en-v1.5`](https://huggingface.co/BAAI/bge-small-en-v1.5), MIT licensed, ~133 MB) into `~/.claude/subrosa/models/`, through your own `curl`, checked against a sha256 pinned in the source. Then it embeds each archived turn and stores the vector beside it, one turn per core — about 9 minutes for a 127,000-turn archive on an M3 Max, under 1.5 GB of memory. It works newest first, so your recent sessions are searchable in the first minute, and it's resumable: Ctrl-C just means the next run finishes the rest. Re-run it after new sessions pile up, since only embedded turns can be ranked — `--semantic` warns you whenever some were left out and tells you how many. `subrosa embed --rebuild` throws away what's stored and starts over, which is the fix if a search ever reports unreadable vectors.
