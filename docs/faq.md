...
Not to us or to anyone else: no cloud, no telemetry, no update checker, and the binary never opens a socket itself. Three network things exist, and all three only pull. The plugin's one-time bootstrap downloads the program from GitHub releases (sha256-verified against checksums in this repo). The optional backup mirror copies a snapshot into a folder you pick — aim it at iCloud or Dropbox and your sync client uploads the snapshot, your choice, off by default, only static snapshots ever land there, never the live database, and it goes out encrypted if you set a mirror passphrase. And the search model comes down from Hugging Face on the first run, by handing three URLs to your own `curl` — a download only, nothing of yours goes up, and `semantic=off` in `~/.claude/subrosa/config` skips it entirely. [Why keyword search](#why-keyword-search-instead-of-embeddings) has the details.
...
1. Nothing. The first session start downloads the model it uses ([`BAAI/bge-small-en-v1.5`](https://huggingface.co/BAAI/bge-small-en-v1.5), MIT licensed, ~133 MB) into `~/.claude/subrosa/models/`, through your own `curl`, checked against a sha256 pinned in the source, and indexes your archive in the background. That run is a separate low-priority process on half your cores — two if you have two or three, and the single one if that's all you have — and it exits when it's done, and it picks up new sessions the same way later — about 9 minutes for a 127,000-turn archive on an M3 Max, under 1.5 GB of memory. It works newest first, so your recent sessions are searchable in the first minute, and it's resumable: killing it just means the next one finishes the rest. `subrosa` (the dashboard) shows how far along it is. `semantic=off` in `~/.claude/subrosa/config` turns the whole thing off — no background run, and no model download from any subrosa process, including a `subrosa embed` you type yourself. A model already on disk keeps working, so an index built before you switched it off still searches. `subrosa embed --rebuild` throws away what's stored and starts over, which is the fix if a search ever reports unreadable vectors.
2. `subrosa search --semantic "why did checkout get slow"` ranks by meaning: a turn can surface without sharing a single word with your query. Turns archived since the last index run aren't ranked yet, so it tells you how many were left out — the run that closes the gap has already started.

Nothing you typed leaves the machine. The download only pulls files down; your turns and your queries are embedded locally, and the query is redacted before the model sees it, same as the archived text was. If the model isn't on disk and can't be fetched — no network, say — nothing breaks: keyword search carries on, the run that didn't finish waits an hour before trying again (then doubles the wait, up to a day), and `--semantic` says how many turns are missing, with the reason the last run gave, instead of quietly falling back to keyword. `subrosa embed` retries right away and prints the three URLs to grab by hand if it still can't. Save a hand-download under the `.part` name it shows you: subrosa checksums those and renames them into place itself, and a file dropped straight at its final name is only checked by size.
