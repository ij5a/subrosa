...
| search.rs | FTS5 query building + result output (incl. `--after`/`--before`/`--tag` filters), plus the opt-in `--semantic` ranking and the `subrosa embed` backfill (newest-first slabs, deduped, one thread per core over a shared `Embedder`, main thread the only DB writer) |
